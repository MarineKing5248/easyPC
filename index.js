...
app.post("/friend-status", async (req, res) => {
  try {
    ...
    if (friendStatus.rows[0]) {
      ...
      } else if (req.session.loggedIn == friendStatus.rows[0].receiver_id) {
        res.json({
...
app.post("/post-wall", async (req, res) => {
  try {
    ...
    const senderData = await queryFunction.fetchUserData(req.session.loggedIn);
...
app.get("/get-wallposts/:otherUserId", async (req, res) => {
  try {
    if (!isNaN(req.params.otherUserId)) {
      ...
      const userData = await queryFunction.fetchUserData(req.session.loggedIn);
      const { firstname } = userData.rows[0];
      res.json({
        wallPostsReceived: wallPosts.rows,
        postsOnWall: true,
...
io.on("connection", function(socket) {
  ...
  console.log(
    `socket with the id ${socket.id} and USERID ${loggedIn} is now connected`
  );
